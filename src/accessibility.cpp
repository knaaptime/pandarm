...
#pragma omp for schedule(guided)
                             ...
                             vector<int64_t> ext_ids)
        {
            ...
            // Set up a mapping between the external node ids and int64_ternal ones
            std::unordered_map<int64_t, int64_t> int64_t_ids(ext_ids.size());
            for (int64_t i = 0; i < ext_ids.size(); i++)
            {
                int64_t_ids.insert(pair<int64_t, int64_t>(ext_ids[i], i));
            }
            ...
            if (dmsradius > 0 && radius <= dmsradius)
            {
                ...
                {
                    dists[i] = dms[graphno][int64_t_ids[srcnodes[i]]];
                }
            }
            else
            {
...
#pragma omp for schedule(guided)
                for (int64_t i = 0; i < srcnodes.size(); i++)
                {
                    ga[graphno]->Range(int64_t_ids[srcnodes[i]], radius,
        ...
        */
            ...
            int64_t graphno)
        {
            if (accessibilityVars.find(category) == accessibilityVars.end() ||
                std::find(aggregations.begin(), aggregations.end(), aggtyp) == aggregations.end() ||
